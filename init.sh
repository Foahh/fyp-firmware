...
sync_git_checkout() {
  local paths=(
    "External/STM32CubeN6"
    "External/fyp-power-measure"
    "External/nanopb"
    "External/stm32-mw-camera"
    "External/stm32-mw-isp"
    "External/stm32-vl53l5cx"
  )
  ...
  git -C "$repo_root" submodule update --init --recursive "${paths[@]}"
}
...
restore_from_lock() {
  ...
  while IFS=$'\t' read -r path url commit mode; do
    ...
    if [ "$mode" = "recursive" ]; then
      git -C "$local_target" submodule update --init --recursive
    fi
  done < "$lock_file"
}
