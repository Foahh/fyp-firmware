...
#undef BUS_I2C1_FREQUENCY
#define BUS_I2C1_FREQUENCY 400000U /* fast mode */
...
static TX_MUTEX BspI2c1Mutex;
static uint32_t BspI2c1MutexCreated = 0;
static TX_MUTEX BspI2c2Mutex;
static uint32_t BspI2c2MutexCreated = 0;
/**
 * @}
 */
...
int32_t BSP_I2C1_Init(void) {
  ...
  if (I2c1InitCounter == 0U) {
    ...
    if (HAL_I2C_GetState(&hbus_i2c1) == HAL_I2C_STATE_RESET) {
      if (!BspI2c1MutexCreated) {
        tx_mutex_create(&BspI2c1Mutex, "bsp_i2c1", TX_INHERIT);
        BspI2c1MutexCreated = 1;
      }
      ...
      if (MX_I2C1_Init(&hbus_i2c1,
                       I2C_GetTiming(HAL_RCC_GetPCLK1Freq(), BUS_I2C1_FREQUENCY)) != HAL_OK) {
...
int32_t BSP_I2C1_WriteReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length) {
  int32_t ret;
  tx_mutex_get(&BspI2c1Mutex, TX_WAIT_FOREVER);
...
int32_t BSP_I2C2_Init(void) {
  ...
  if (I2c2InitCounter == 0U) {
    ...
    if (HAL_I2C_GetState(&hbus_i2c2) == HAL_I2C_STATE_RESET) {
      if (!BspI2c2MutexCreated) {
        tx_mutex_create(&BspI2c2Mutex, "bsp_i2c2", TX_INHERIT);
        BspI2c2MutexCreated = 1;
      }
#if (USE_HAL_I2C_REGISTER_CALLBACKS == 0)
      /* Init the I2C2 Msp */
...
int32_t BSP_I2C2_WriteReg(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length) {
  int32_t ret;
  tx_mutex_get(&BspI2c2Mutex, TX_WAIT_FOREVER);
